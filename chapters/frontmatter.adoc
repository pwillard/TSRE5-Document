...
[NOTE]
Any line in the `Settings.txt` file that begins with `#` is "commented out" and is therefore *DISABLED*.  So, to *enable* the AASAMPLES option in the first line, you would edit the `SETTINGS.TXT` file the TSRE folder with  the text editor of your choice and delete the `#`  in front of AASAMPLES.
