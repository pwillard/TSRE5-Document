...
8.004 adds the following:
 ...
 Offset from track centerline added for signal and milepost placement (sigOffset setting)
 ...
 TDB Fixes
 Address multiple paths for include files
 Considers engine/wagon mass values in excess of 999 tons as pounds and converts them to tons
 Reduced informational logging entries
 Adds new Settings token MapAPIKey (optionally separates this from being included in the URL)
 Mass calculations into metric tonnes for commonly supported units (lb, kg, t-us, t-uk)
 Reduced logging messages while debugOutput = false

  New settings tokens to disable ORTS texture handling:
   imageSubstitution = false 
   imageUpgrade = false
 Allow image substitution to be disabled by setting imageSubstitution = false and make sure that it will not apply to the TERRTEX files
