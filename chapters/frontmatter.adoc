...
|	imageMapsUrl 	|	 http://api.mapbox.com/v4/mapbox.satellite/{lon},{lat},{zoom}/{res}x{res}.png?access_token	|	EXAMPLE: http://maps.googleapis.com/maps/api/staticmap?center={lat},{lon}&zoom={zoom}&size={res}x{res}&maptype=satellite&key  `Universal URL setting for using google maps`
...
[NOTE]
It the comment says *new* and nothing else... we still need a good explanation of what it's supposed to do.
