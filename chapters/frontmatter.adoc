...
The list of items above was extracted from the TSRE5 Source Code.  The list below is an attempt to describe each option, where possible.
...
|	imageMapsUrl 	|	 http://api.mapbox.com/v4/mapbox.satellite/{lon},{lat},{zoom}/{res}x{res}.png?access_token	|	http://maps.googleapis.com/maps/api/staticmap?center={lat},{lon}&zoom={zoom}&size={res}x{res}&maptype=satellite&key | `Universal URL setting for using google maps`
